 ...
 */
ER
BSP_AUDIO_OUT_Pause(AUDIO_Handle_t *haudio)
{
	if(audio_drv->Pause(AUDIO_I2C_ADDRESS) != 0){
		...
		audio_dmapause(haudio, AUDIO_OUT_BLOCK);
		return E_OK;
	}
}
 ...
 */
ER
BSP_AUDIO_OUT_Resume(AUDIO_Handle_t *haudio)
{
	if(audio_drv->Resume(AUDIO_I2C_ADDRESS) != 0){
		...
		audio_dmaresume(haudio, AUDIO_OUT_BLOCK);
		return E_OK;
	}
}
