 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: adc.c 698 2017-07-11 20:52:51Z roi $
 */
/*
 * 
 *  ADCドライバ関数群
...
#define INIT_IER                (ADC_IER_ADRDYIE | ADC_IER_EOSMPIE | ADC_IER_EOCIE | \
								 ADC_IER_EOSEQIE | ADC_IER_OVRIE   | ADC_IER_AWDIE | ADC_IER_EOCALIE)

#define INIT_ISR                (ADC_ISR_ADRDY | ADC_ISR_EOSMP | ADC_ISR_EOC | \
								 ADC_ISR_EOSEQ | ADC_ISR_OVR   | ADC_ISR_AWD | ADC_ISR_EOCAL)
...
adc_init(ID portid, ADC_Init_t* pini)
{
	 ...
	 *  ADCのプリスケーラ設定(コモン設定)
	 */
	if((hadc->Init.ClockPrescaler & ADC_CFGR2_CKMODE) != 0)
		...
		sil_modw_mem((uint32_t *)(TADR_ADC_BASE+TOFF_ADC_CCR), ADC_CCR_PRESC, hadc->Init.ClockPrescaler);
	}
	...
	sil_modw_mem((uint32_t *)(TADR_ADC_BASE+TOFF_ADC_CCR), ADC_CCR_LFMEN, hadc->Init.LowPowerFrequencyMode);
	 ...
	 *  オーバーサンプリングモード設定
	 */
	if(hadc->Init.OversamplingMode != ADC_OVR_SAMPLING_DISABLE){
		sil_modw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR2), (ADC_CFGR2_OVSR | ADC_CFGR2_OVSS | ADC_CFGR2_TOVS),
			(hadc->Init.OversamplingRatio | hadc->Init.OversamplingRightBitShift | hadc->Init.OversamplingTriggeredMode));
		sil_orw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR2), ADC_CFGR2_OVSE);
	}
	else{
		/*
		 *  オーバーサンプリング設定無効化
		 */
		if((sil_rew_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR2)) &ADC_CFGR2_OVSE) != 0)
			sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR2), ADC_CFGR2_OVSE);
	}
...
adc_deinit(ADC_Handle_t* hadc)
{
	...
	if(ercd == E_OK){
		 ...
		 * リセットボルテージレギュレータ
		 */
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CR), ADC_CR_ADVREGEN);

		/*
		 *  リセットCFGR1/CGFR2
		 */
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR1), (INIT_CFGR1 | ADC_CFGR1_AWDCH  | ADC_CFGR1_AWDEN |
															ADC_CFGR1_AWDSGL  | ADC_CFGR1_RES | ADC_CFGR1_DMAEN));
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR2), (ADC_CFGR2_TOVS  | ADC_CFGR2_OVSS  | ADC_CFGR2_OVSR |
		 ...
		 *  リセットTR
		 */
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_TR), (ADC_TR_LT | ADC_TR_HT));

		/*
		 *  リセットCALFACT
		 */
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CALFACT), ADC_CALFACT_CALFACT);
...
adc_start_int(ADC_Handle_t* hadc)
{
	...
	if(ercd == E_OK){
		 ...
		 *  割込みクリア
		 */
		sil_wrw_mem((uint32_t *)(hadc->base+TOFF_ADC_ISR), (ADC_ISR_EOC | ADC_ISR_EOSEQ | ADC_ISR_OVR));
		...
		sil_orw_mem((uint32_t *)(hadc->base+TOFF_ADC_IER), (ADC_IER_EOSEQIE | ADC_IER_OVRIE));
...
adc_end_int(ADC_Handle_t* hadc)
{
	...
	if(ercd == E_OK){
		 ...
		 *  転送、エラー割込み停止
		 */
		sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_IER), (ADC_IER_EOCIE | ADC_IER_EOSEQIE | ADC_IER_OVRIE));
...
adc_handler(ADC_Handle_t* hadc)
{
	...
	if(((isr & ier) & (ADC_ISR_EOC | ADC_ISR_EOSEQ)) != 0){
	    /*
		 *  コンバージョン終了状態に移行
		 */
	    ...
	    if((cr1 & ADC_CFGR1_EXTEN) == 0 && hadc->Init.ContinuousConvMode == ADC_CONTINUOUS_DISABLE){
		    ...
		    if((isr & ADC_ISR_EOSEQ) != 0){
				...
				if((sil_rew_mem((uint32_t *)(hadc->base+TOFF_ADC_CR)) & ADC_CR_ADSTART) == 0){
					 ...
					 *  割込みマスク
					 */
					sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_IER), (ADC_IER_EOCIE | ADC_IER_EOSEQIE));
					hadc->status = ADC_STATUS_BUSY_EOC;
				}
				else{	/* インターナルエラー */
					hadc->status = ADC_STATUS_ERROR;
	    ...
	    if(hadc->Init.LowPowerAutoWait != ADC_LOWAUTOWAIT_ENABLE){
			sil_wrw_mem((uint32_t *)(hadc->base+TOFF_ADC_ISR), (ADC_ISR_EOC | ADC_ISR_EOSEQ));
		}
	}
	 ...
	 *  アナログウォッチドック判定
	 */
	if((isr & ADC_ISR_AWD) != 0 && (ier & ADC_IER_AWDIE) != 0){
		/*
		 *  ウォッチドック発生状態に変更
		 */
		 ...
		 *  ウォッチドック割込みクリア
		 */
		sil_wrw_mem((uint32_t *)(hadc->base+TOFF_ADC_ISR), ADC_ISR_AWD);
...
adc_setupchannel(ADC_Handle_t* hadc, ADC_ChannelConf_t* sConfig)
{
	 ...
	 *  チャネル有効設定
	 */
	sil_orw_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_IOPENR), 1<<clkpin);
	sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_IOPENR));
	...
	if(sConfig->Rank != ADC_RANK_NONE){
...
#if defined(ADC_CHSELR_CHSEL16) && defined(ADC_CCR_VLCDEN)
		/*
		 *  VLCDチャネル設定
		 */
		if(sConfig->Channel == ADC_CHANNEL_VLCD){
			sil_orw_mem((uint32_t *)(TADR_ADC_BASE+TOFF_ADC_CCR), ADC_CCR_VLCDEN);
		}
#endif
	}
	else{
		 ...
		 */
		if(sConfig->Channel == ADC_CHANNEL_VREFINT){
			sil_andw_mem((uint32_t *)(TADR_ADC_BASE+TOFF_ADC_CCR), ADC_CCR_VLCDEN);
		}
#endif
	}
	return E_OK;
...
adc_setupwatchdog(ADC_Handle_t* hadc, ADC_AnalogWDGConf_t* AnalogWDGConfig)
{
	 ...
	 *  割込み設定
	 */
	sil_modw_mem((uint32_t *)(hadc->base+TOFF_ADC_IER), ADC_IER_AWDIE, AnalogWDGConfig->ITMode);

	/*
	 *  モード設定
	 */
	sil_andw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR1), (ADC_CFGR1_AWDSGL | ADC_CFGR1_AWDEN | ADC_CFGR1_AWDCH));

	/* Set the analog watchdog enable mode */
	sil_orw_mem((uint32_t *)(hadc->base+TOFF_ADC_CFGR1), (AnalogWDGConfig->WatchdogMode |
									(AnalogWDGConfig->Channel & ADC_CFGR1_AWDCH)));
