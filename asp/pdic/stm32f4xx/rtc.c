 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: rtc.c 698 2017-12-05 18:17:44Z roi $
 */
/*
 *
 * RTC用デバイス操作関数群の定義
...
#define RCC_CFGR_RTCPRE         (RCC_CFGR_RTCPRE_0 | RCC_CFGR_RTCPRE_1 | RCC_CFGR_RTCPRE_2 | RCC_CFGR_RTCPRE_3 | RCC_CFGR_RTCPRE_4)
...
static void (*rtcalarmA_callback)(void);	/* rtc alarmA callback */
static void (*rtcalarmB_callback)(void);	/* rtc alarmB callback */
...
rtc_init(intptr_t exinf)
{
...
#if RCC_RTCCLKSOURCE_DEF == RCC_RTCCLKSOURCE_LSE
    /* LSEスタート */
	sil_wrb_mem((uint8_t *)RCC_BDCR_BYTE0_ADDRESS, RCC_LSE_ON);
    /* Check the LSE State */
	tick = 0;
...
#elif RCC_RTCCLKSOURCE_DEF == RCC_RTCCLKSOURCE_LSI
	/* LSIスタート */
	sil_wrw_mem((uint32_t *)RCC_CSR_LSION_BB, 1);
	...
	if(tmpreg == 0 || tmpreg != RCC_RTCCLKSOURCE_DEF){
		/*
		 *  クロック電源設定
		 */
		tmpreg = (sil_rew_mem((uint32_t *)(TADR_RCC_BASE+TOFF_RCC_BDCR)) & ~(RCC_BDCR_RTCSEL));
